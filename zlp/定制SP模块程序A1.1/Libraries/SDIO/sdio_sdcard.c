...
SD_Error SD_Init(void)
{
  ...
  SDIO_InitStructure.SDIO_ClockDiv       = SDIO_TRANSFER_CLK_DIV;            //wang
  ...
  if (errorstatus == SD_OK)
  {
    ...
    #ifdef PRINT_DEBUG
        if (errorstatus != SD_OK)
		    printf("Get SD information failed!\r\n");
	#endif  /*PRINT_DEBUG*/
  }
...
SD_Error SD_WaitReadOperation(void)
{
  ...
  //等待dma传输结束
  while ((SD_DMAEndOfTransferStatus() == 0) && (TransferEnd == 0) && (TransferError == SD_OK))
  {
      timeout--;
	  if (timeout == 0)
	  {
	      #ifdef SD_DEBUG
            printf("Wait DMA read end!\r\n");
	      #endif  /*SD_DEBUG*/
...
SD_Error SD_WaitWriteOperation(void)
{
  ...
  while ((SD_DMAEndOfTransferStatus() == RESET) && (TransferEnd == 0) && (TransferError == SD_OK))
  {
	  ...
	  {
	      #ifdef SD_DEBUG
            printf("Wait write DMA end out time!\r\n");
	      #endif  /*SD_DEBUG*/
...
SD_Error SD_WaitTransferOK(void)
{
    ...
    while(SD_GetStatus() != SD_TRANSFER_OK)
	{
	    ...
	    {
	        #ifdef SD_DEBUG
              printf("Wait SD_TRANSFER_OK out time!\r\n");
	        #endif  /*SD_DEBUG*/
