...
static uint8_t UsartFrameHeader[] = {0x41, 0xC8, 0, 0xAA, 0xAA, 0xFD, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, FRAME_CONFIG_ID};	//无线--串口帧帧头
...
//	OSTimeDly(WaitForPower);	//延时启动任务，10s
	...
	{
//		sDWT.ShortAddr = (uint16_t)DWT_ReadFromFlash >> UWB_16BIT_ADDR_IDX;
...
int8_t DWT_WakeUp(void)
{
	...
	{
		HAL_GPIO_WritePin(D_WAKEUP_GPIO_Port, D_WAKEUP_Pin, GPIO_PIN_SET);
		DECA_Sleep(2);
		...
		DWT_SetEUI((uint8_t *)&sDWT.ChipID);
	}
	else
	{
		return DWT_SUCCESS;
...
void UWB_ID_Config(void *pdata)
{
	...
	for(;;)
	{
		...
		if ((TagStatusReg = DWT_Read32BitReg(SYS_STATUS_ID)) & SYS_STATUS_RXFCG)	//成功接收
		{
			...
			DWT_Write32BitReg(SYS_STATUS_ID, SYS_STATUS_RXFCG);		//清除寄存器标志位
			...
			if(FrameLen <= RX_BUF_LEN_MAX)
			{
				...
				if((FrameCotrl & FRAME_CTRL_BYTE_ADDR_LEN_BIT) == FRAME_CTRL_BYTE_DEST_ID_64BIT)	//64Bit数据帧
				{
					...
					if((sTagConfig.ChipID == DestAddr) || RxBuffer[FRAME_64BIT_DATA_IDX + USART_FRAME_TYPE_IDX] == USART_FRAME_QUERY)	//接收地址是自己的或者是查询帧
					{
						UsartLen = 0;
						if(FrameLen > UsartShortFrameLen)
						{
						...
						else
						{
							...
							{
								TAG_ConfigID_ACK(USART_FRAME_REPEAT_ACK);
								TagUsartCrcRepeatNum++;
								
							}
							else
							{
								TAG_ConfigID_ACK(USART_FRAME_ERR_ACK);
								TagUsartCrcRepeatNum = 0;
							}
						}
					}	
...
void TAG_ConfigID(uint8_t frametype)
{
	...
	switch(frametype)
	{
		...
		case USART_FRAME_DEINIT:
			sDWT.ShortAddrFlag = 0;
			sDWT.ShortAddr = 0;
			break;
		case USART_FRAME_QUIT:
			KeepInConfig = 0;
			break;
	}
}
...
void TAG_ConfigID_ACK(uint8_t frametype)
{
	...
	switch(frametype)
	{
		case USART_FRAME_QUERY:
		{
			...
			{
				uint8_t DataWriteOffect = USART_ACK_CHIPID_IDX;	//初始化数据写入偏移量
				
				UsartFramePayLoad[USART_ACK_TYPE_IDX] = USART_FRAME_INFO_ALL;		//写入串口帧类别--回复全部信息
				...
				DataPayLoadLength = DataWriteOffect - USART_ACK_CHIPID_IDX + 2;	//串口负载长度：数据写入偏移量 - 负载起始偏移量 + CRC长度
			...
			{
				UsartFramePayLoad[USART_ACK_TYPE_IDX] = USART_FRAME_INFO_CHIPID;		//写入串口帧类别
				
				WriteToMsg(UsartFramePayLoad, (uint8_t *)&sTagConfig.ChipID, USART_ACK_CHIPID_IDX, CONFIG_CHIPID_LEN, 1);		//写入ChipID
				
				DataPayLoadLength = CONFIG_CHIPID_LEN + 2;	//串口负载长度：ChipID长度 + CRC长度
				WriteToMsg(UsartFramePayLoad, (uint8_t *)&DataPayLoadLength, USART_FRAME_LEN_IDX, 2, 1);		//写入帧负载长度
				
				Crc = GetCRC(UsartFramePayLoad, 0, DataPayLoadLength + 2);
				WriteToMsg(UsartFramePayLoad, (uint8_t *)&Crc, USART_ACK_CHIPID_IDX + CONFIG_CHIPID_LEN, 2, 1);		//写入CRC
			}
		...
		case USART_FRAME_SET_ALL:
		{
			UsartFramePayLoad[USART_ACK_TYPE_IDX] = USART_FRAME_SET_ALL_ACK;		//写入串口帧类别--更新全部信息完成
			
			Crc = GetCRC(UsartFramePayLoad, 0, 2);
			WriteToMsg(UsartFramePayLoad, (uint8_t *)&Crc, USART_FRAME_CRC_IDX, 2, 1);		//写入CRC
			
			DWT_WriteTxData(6, UsartFramePayLoad, sizeof(UsartFrameHeader));								//写入发送数据
			DWT_WriteTxfCtrl(sizeof(UsartFrameHeader) + 6, 0, 0);														//设定发送长度
			break;
		}
		case USART_FRAME_REPEAT_ACK:
		{
			UsartFramePayLoad[USART_ACK_TYPE_IDX] = USART_FRAME_REPEAT;		//写入串口帧类别--重发帧
		...
		case USART_FRAME_ERR_ACK:
		{
			UsartFramePayLoad[USART_ACK_TYPE_IDX] = USART_FRAME_ERR;		//写入串口帧类别--CRC错误帧
		...
		case USART_FRAME_ERR:
		{
			while(1)
			{
				HAL_GPIO_TogglePin(LED_PIN_Port, LED5_PIN | LED6_PIN);
				DECA_Sleep(100);
			}
		}
		case USART_FRAME_DEINIT:
		{
			UsartFramePayLoad[USART_ACK_TYPE_IDX] = USART_FRAME_DEINIT_ACK;		//写入串口帧类别--CRC错误帧
		...
		default:
		{
			UsartFramePayLoad[USART_ACK_TYPE_IDX] = frametype;		//写入串口帧类别--ACK回复帧
...
void TAG_ReadAllConfigInfo(void)
{
	...
	DataReadOffect += ReadStringFromMsg(sTagConfig.Title, RxBuffer, FRAME_64BIT_DATA_IDX + DataReadOffect);				//读取并保存职务，更新数据读取偏移量
	DataReadOffect += ReadStringFromMsg(sTagConfig.WorkSpace, RxBuffer, FRAME_64BIT_DATA_IDX + DataReadOffect);		//读取并保存工作区，更新数据读取偏移量
	
	ReadStringFromMsg(sTagConfig.Perm, RxBuffer, FRAME_64BIT_DATA_IDX + DataReadOffect);													//读取并保存权限
}
...
void TAG_SaveConfigInfoToFlash(void)
{
	...
	FLASH_Erase(FLASH_TYPEERASE_PAGES, TAG_INFO_ADDR);
...
/*******************************************************************************************
* 函数名称：ReadConfigInfoFromFlash(void)
* 功能描述：读取标签配置信息
* 入口参数：无
* 出口参数：无
* 使用说明：无
